...
class Program
{
    ...
    {
        string[] complexityOptions = new string[]
        {
        "1. Легкий (тільки маленькі букви і цифри)",
        "2. Середній (маленькі букви, великі букви і цифри)",
        "3. Тяжкий (маленькі букви, великі букви, цифри та символи)"
        };

        int complexityChoice = 0;
        ...
        while (true)
        {
            ...
            {
                if (i == complexityChoice)
                {
                    Console.BackgroundColor = ConsoleColor.DarkBlue;
                    Console.WriteLine(complexityOptions[i]);
                    Console.ResetColor();
                }
                else
                {
            ...
            switch (key)
            {
                case ConsoleKey.UpArrow:
                    ...
                    break;
                case ConsoleKey.DownArrow:
                    if (complexityChoice < complexityOptions.Length - 1)
                        complexityChoice++;
                    break;
                case ConsoleKey.Enter:
                    return GetPasswordCharactersForComplexity(complexityChoice);
            }
        }
    }

    static string GetPasswordCharactersForComplexity(int complexityChoice)
    {
        switch (complexityChoice)
        {
            ...
            case 2:
                return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^&*()";
            default:
                throw new ArgumentOutOfRangeException("Невідомий вибір складності.");
        }
    }
